...
class Engine_aiki {
    ...
    public function layout() {
        ...
        $this->createWidget($widget_group); //@TODO createWigdet will accept array of object.
    }
    ...
    private function createWidget($widget_group) {
        ...
        foreach ($widget_result as $widget) {
            ...
            if ($widget->is_father)    {
                ...
                $son_widgets = $aiki->widgets->get_candidate_widgets($widget->id);
    ...
    private function inline_widgets($widget){
        ...
        if (preg_match_all('/\(\#\(widget\:(.*)\)\#\)/Us', $widget, $matches)){
            foreach ($matches[1] as $widget_id) {
                $widget_id= $aiki->widgets->get_widget_id($widget_id);
