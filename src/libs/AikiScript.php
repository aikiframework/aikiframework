	...
	public function parser($text) {
		...
		do {
			...
			switch ($word) {				
				...
				// counters and odds
				case "setcounter":
					$this->setcounter($this->mtoken($rest)); 
					break;
				case "counter":
					$php_output = $this->counter($rest);   
					break;
				case "odd":
					$php_output = $this->odd(); 
					break;
				case "mod":
					$php_output = $this->mod($rest); 
					break;
				// getinfo
				case "getinfo":
	...
	function meval($text) {
		...
		if (preg_match('/^\$_(GET|POST|SESSION|REQUEST|COOKIE)\[(.*)\]$/', $text, $cap)) {
			...
			switch ($cap[1]) {
				 ...
				 case "SESSION":
					$text = isset($_SESSION[$key]) ? $_SESSION[$key] : "" ; break;
				 case "COOKIE":
					$text = isset($_COOKIE[$key]) ? $_COOKIE[$key] : "" ; break;
				 default:
					$text = "";
			}
		}
