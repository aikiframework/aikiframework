			...
			if (!isset($get_permission_and_man_info[1]) or $get_permission_and_man_info[1] == $membership->permissions or $membership->group_level < $get_group_level){
				...
				switch ($switcher){
					...
					case "staticselect":
						...
						if ($intwalker[2] == "custome" and $intwalker[3]){
							...
							foreach ($explodeStaticSelect as $option){
								...
								$form .= '<option value='.$optionsieds['1'].'>'.$optionsieds['0'].'</option>';
							}
							$form .= '</select>';
						}
						$form .= "</td></tr>";
					...
					case "selection":
						...
						if ($aquery){
							foreach ( $aquery as $mini_selection )
							{
								...
								$form .= "<option value=\"$id\" "; if ($_POST[$intwalker[0]] == $id){$form .= "selected";} $form .= ">$name</option>";
							}
						}
						$form .= ("</select></td></tr>");
						break;
					...
					case "ckeditor":
						...
						$form .= "<tr><td>$intwalker[1]</td><td><textarea dir=\"$get_permission_and_man_info[3]\" name=\"$intwalker[0]\">".$_POST[$intwalker['0']]."</textarea>";
						...
						break;
							...
							case "autofiled":
								switch ($intwalker[2]){
									...
									case "insertedby":
										$form .= ("<input type=\"hidden\" name=\"$intwalker[0]\" value=\"$membership->full_name\">");
										break;

									case "insertedby_username":
										$form .= ("<input type=\"hidden\" name=\"$intwalker[0]\" value=\"$membership->username\">");
										break;
									...
									case "EditingHistory":
										...
										$form .= ("<input type=\"hidden\" name=\"$intwalker[0]\" value=\"- Inserted By $membership->full_name on $datetime <br />\">");
										break;
